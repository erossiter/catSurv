...
test_that("EAP estimation calculates correctly", {
  ...
  for(i in 1:10){
    ...
    tpm_cat@answers <- unlist(polknowMT[i, ])
  ...
  expect_equal(estimateThetas(tpm_cat, polknowMT[1:10, ]), indv_tpm)
...
test_that("MAP estimation calculates correctly", {
  ltm_cat@estimation <- tpm_cat@estimation <- grm_cat@estimation <- gpcm_cat@estimation <- "MAP"
