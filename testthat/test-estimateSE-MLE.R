...
test_that("estimateSE calculates correctly for MLE", {
  ...
  for(j in 1:nrow(ltm_data)){
    ...
    RSE <- semTheta(thEst = theta.ltm, it = it.ltm, x = ltm_data[j,], method = "ML", 
                    priorDist = "norm", priorPar = c(0,1), parInt = c(-10, 10, 100))
    if(j == 69 || j == 78){ # 69, 78 all.extreme == TRUE
      RSE <- CatSE
    }
  ...
  for(j in 1:nrow(tpm_data)){
    ...
    RSE <- semTheta(thEst = theta.tpm, it = it.tpm, x = tpm_data[j,], method = "ML", 
                    priorDist = "norm", priorPar = c(0,1), parInt = c(-10, 10, 100))
    if(j == 69 || j == 78){ # 69, 78 all.extreme == TRUE
      RSE <- CatSE
    }
  ...
  for(j in 1:nrow(poly_data)){
    ...
    RSE <- semTheta(thEst = theta.poly, it = it.poly, x = as.numeric(poly_data[j,] - 1), 
                    model = "GRM", method = "ML", priorDist = "norm", priorPar = c(0,1), 
                    parInt = c(-10, 10, 100))
    expect_equal(abs(CatSE - RSE)/CatSE, 0, tolerance = .001)
  }
  # 
