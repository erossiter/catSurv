...
test_that("expectedObsInf calculates correctly", {
  ...
  expectedObsInf_test_CatR <- function(poly){
    ...
    if(poly == FALSE){
        ...
        for(i in 1:nrow(binary_data)){
          ...
          ourMEI[i] <- expectedObsInf(binary_cat, 6)
        
          # This command returns the expected information (EI) for a given item
          # (under dichotomous and polytomous IRT models),
          # as used for Maximum Expected Information (MEI) criterion.
  ...
  expect_true(all.equal(expectedObsInf_test_CatR(FALSE), rep(0, 100), tolerance = .01))
  expect_true(all.equal(expectedObsInf_test_CatR(TRUE), rep(0, 100), tolerance = .01))
})
